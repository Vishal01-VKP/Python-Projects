...
class Chess_App(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.Chess_Board = {f"button{n+1}":QtWidgets.QPushButton(self) for n in range(64)}
        ...
        for var1 in range(8):
            if var1 % 2 == 0:
                for i in range(0,8,2):
                    self.Chess_Board[f"button{(var1*8)+i+1}"].setStyleSheet("QPushButton {background-color : lightgreen}")
        ...
        for x in range(8):
            for y in range(8):
                if self.final_list[x][y].isupper():
                    ...
                    self.Chess_Board[f"button{(y*8)+x+1}"].setIconSize(QtCore.QSize(45,45))
